  ...
  const submit = async () => {
    ...
    try {
      const res = await apiFetch('/v1/auth/signup', {
        method: 'POST',
        body: JSON.stringify({ phone_number: '+' + full, email: email || undefined }),
      });
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100dvh', padding: '0 0 32px' }}>
      ...
      <div style={{ flex: 1, padding: '32px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          <div style={{ fontSize: 22, fontWeight: 800, fontFamily: 'var(--font-display)', marginBottom: 8 }}>Sign up</div>
          <div style={{ fontSize: 14, color: 'var(--text-muted)' }}>Enter your Dominica number to get started</div>
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <input
              type="tel" inputMode="numeric" placeholder="767 000 0000"
        ...
        {error && <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.2)', color: 'var(--accent-coral)', fontSize: 13 }}>{error}</div>}
